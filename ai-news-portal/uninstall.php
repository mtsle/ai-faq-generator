 ...
 *
 * SIEC: w instalacji wielowitrynowej sprzatanie biegnie w petli po WSZYSTKICH
 * witrynach, bo tabela, wpisy, terminy i opcje sa per-blog. Bez tego 'BEZ SLADU'
 * z tytulu obowiazywaloby tylko na jednej witrynie. Wzorzec i rozmiar strony
 * wynikow sa te same co w uninstall.php wtyczki 1 — obie maja sprzatac tak samo.
 *
...
if ( ! function_exists( 'ainp_uninstall_cleanup_site' ) ) {
	/**
	 * Sprzata JEDNA witryne (biezacy blog).
	 *
	 * Wydzielone do funkcji, bo w multisite trzeba to wykonac dla kazdego bloga
	 * osobno: tabela, wpisy, terminy i opcje sa per-blog, a `switch_to_blog()`
	 * podmienia `$wpdb->prefix` (wp_2_, wp_3_, ...). Nazwa tabeli MUSI wiec
	 * powstawac wewnatrz tej funkcji, nie raz na starcie pliku.
	 *
	 * Prefiks `ainp_` w nazwie i oslona `function_exists()` chronia przed kolizja:
	 * plik laduje sie do globalnej przestrzeni nazw, obok uninstall.php innych wtyczek.
	 *
	 * @return void
	 */
	function ainp_uninstall_cleanup_site() {
		global $wpdb;
...
// --- Wywolanie: pojedyncza witryna albo cala siec --------------------------
//
// W multisite wtyczka trzyma osobny komplet tabeli, wpisow, terminow i opcji
// w KAZDYM blogu, wiec sprzatanie tylko biezacego zostawialoby reszte sieci
// zasmiecona — wbrew obietnicy 'BEZ SLADU' z naglowka tego pliku i wbrew
// wtyczce 1 z tej samej paczki, ktora siec obsluguje w pelni.
//
// Witryny bierzemy STRONICUJAC. Jedno wywolanie z `number` — jakakolwiek liczba —
// zostawia w wiekszej sieci czesc witryn nieposprzatanych i nikt sie o tym nie
// dowiaduje. Petla konczy sie na pierwszej stronie krotszej niz pelna, a
// `fields => ids` trzyma w pamieci same liczby, wiec strona moze byc mala.
if ( function_exists( 'is_multisite' ) && is_multisite()
	&& function_exists( 'get_sites' ) && function_exists( 'switch_to_blog' ) && function_exists( 'restore_current_blog' ) ) {

	$ainp_strona       = 500;   // Rozmiar strony wynikow.
	$ainp_przesuniecie = 0;

	do {
		$ainp_site_ids = (array) get_sites(
			array(
				'number' => $ainp_strona,
				'offset' => $ainp_przesuniecie,
				'fields' => 'ids',
			)
		);

		foreach ( $ainp_site_ids as $ainp_site_id ) {
			switch_to_blog( (int) $ainp_site_id );
			ainp_uninstall_cleanup_site();
			restore_current_blog();
		}

		$ainp_pobrano       = count( $ainp_site_ids );
		$ainp_przesuniecie += $ainp_pobrano;

		// Warunek zejscia: strona krotsza niz pelna. Pusta strona tez go spelnia,
		// wiec `get_sites()` oddajace `false` albo pustke konczy petle, a nie kreci nia.
	} while ( $ainp_pobrano >= $ainp_strona );
} else {
	ainp_uninstall_cleanup_site();
}
